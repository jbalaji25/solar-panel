...
class App {
  ...
  constructor(
    ...
    this.addEventListeners(clickHandler);
  }
  ...
  onTouchUp(clickHandler?: (item: { image: string; text: string }, index: number) => void, e?: MouseEvent | TouchEvent) {
    this.isDown = false;
    this.onCheck();
    ...
    if (e && clickHandler) {
      ...
      // If we didn't move much, consider it a click
      if (Math.abs(this.start - x) < 5) {
        this.onClick(x, y, clickHandler);
      }
    }
  }

  onClick(x: number, y: number, clickHandler: (item: { image: string; text: string }, index: number) => void) {
    const mouse = {
      x: (x / this.screen.width) * 2 - 1,
      y: -(y / this.screen.height) * 2 + 1
    };

    // Find the closest media to the mouse click
    // Convert mouse NDC to world space approximately or check plane positions
    let closestMedia = null;
    let minDistance = Infinity;

    this.medias.forEach((media, index) => {
      // In this specific gallery, we can check proximity in NDC or viewport space
      // For simplicity, let's check distance to the plane's current position
      const plane = media.plane;

      // Calculate approximate screen position of the plane
      const viewPos = {
        x: (plane.position.x / (this.viewport.width / 2)),
        y: (plane.position.y / (this.viewport.height / 2))
      };

      const dist = Math.sqrt(Math.pow(mouse.x - viewPos.x, 2) + Math.pow(mouse.y - viewPos.y, 2));

      // Only consider if it's within a reasonable range (plane scale)
      const threshold = (plane.scale.x / this.viewport.width) * 2;
      if (dist < minDistance && dist < threshold) {
        minDistance = dist;
        closestMedia = { data: this.mediasImages[index], index: index % (this.mediasImages.length / 2) };
      }
    });

    if (closestMedia) {
      clickHandler(closestMedia.data, closestMedia.index);
    }
  }
  ...
  addEventListeners(clickHandler?: (item: { image: string; text: string }, index: number) => void) {
    this.boundOnResize = this.onResize.bind(this);
    this.boundOnWheel = this.onWheel.bind(this);
    this.boundOnTouchDown = this.onTouchDown.bind(this);
    this.boundOnTouchMove = this.onTouchMove.bind(this);
    this.boundOnTouchUp = this.onTouchUp.bind(this, clickHandler);
